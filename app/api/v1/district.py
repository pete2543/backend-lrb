...
def get_provinces(db: Session = Depends(get_db)):
    provinces = db.query(LRB_Distrcts.province_code, LRB_Distrcts.province).distinct().all()
...
def get_amphoes(province_code: str, db: Session = Depends(get_db)):
    amphoes = db.query(LRB_Distrcts.amphoe, LRB_Distrcts.amphoe_code).filter(LRB_Distrcts.province_code == province_code).distinct().all()
...
def get_subdistricts(amphoe_code: str, db: Session = Depends(get_db)):
    subdistricts = db.query(LRB_Distrcts.district,LRB_Distrcts.district_code).filter(LRB_Distrcts.amphoe_code == amphoe_code).distinct().all()
